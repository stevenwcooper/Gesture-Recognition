...
void HMM::Test_HMM(vector<int> X, vector<int> Y, vector<int> Z)
{
	...
	{
		double gestureRecThreshold = gestureList.at(i).gestureThreshold;
		...
		for(size_t j = 0; j < ATestBinned.data.size(); j++)
		{
			...
			tempRecGesture = pr_hmm(ATestBinned.data.at(j), gestureList.at(i).A, tranverse(gestureList.at(i).B), gestureList.at(i).Pi);
			if(tempRecGesture > gestureRecThreshold)
			{
				//tempRecGesture = (100 / gestureRecThreshold) * tempRecGesture;
...
{
	string filepath = "newdata/test/circle";
	Matrix3D data = get_xyz_data("newdata/test", "circle");
