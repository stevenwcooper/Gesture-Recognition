...
{
	aGesture = Gestures();
	gestureList = aGesture.getGestures();
}
...
void HMM::updateGestures()
{
	aGesture.updateGestures();
	gestureList = aGesture.getGestures();
}

gesture HMM::returnCurrentGesture()
{
	return aGesture.getCurrentGesture();
}

void HMM::Test_HMM(vector<int> X, vector<int> Y, vector<int> Z)
{
	Matrix3D testing = toMatrix3D(X,Y,Z);
	//Initialize

	double recGesture = -DBL_MAX;
	int irecGesture = -1;
	for(size_t i = 0; i < gestureList.size(); i++)
	{
		double gestureRecThreshold = gestureList.at(i).gestureThreshold - (gestureList.at(i).gestureThreshold * 0.05);

		Matrix2D ATestBinned = get_point_clusters(testing, gestureList.at(i).centroids, 3);

		//printf("\n\n******************************************************************************\n");
		//printf("Testing %i sequences for a log likelihood greater than %f\n", ATestBinned.data.size(), gestureRecThreshold);
		//printf("********************************************************************************\n\n");

		for(size_t j = 0; j < ATestBinned.data.size(); j++)
		{
			double tempRecGesture;
			tempRecGesture = viterbi(ATestBinned.data.at(j), gestureList.at(i).A, tranverse(gestureList.at(i).B), gestureList.at(i).Pi);
			if(tempRecGesture > gestureRecThreshold)
			{
				if(tempRecGesture > recGesture)
				{
					recGesture = tempRecGesture;
					irecGesture = ((int)i);
				}
			}
		}
	}
	if(recGesture != -DBL_MAX)
	{
		if(aGesture.getGestureType() == TWO_HAND)
		{
			if(aGesture.getCurrentGesture().GestureName != "") aGesture.setCurrentGesture2(gestureList.at(irecGesture));
			else if(aGesture.getCurrentGesture().GestureName != gestureList.at(irecGesture).GestureName) aGesture.setCurrentGesture(gestureList.at(irecGesture));
		}
		else
			if(aGesture.getCurrentGesture().GestureName != gestureList.at(irecGesture).GestureName)	aGesture.setCurrentGesture(gestureList.at(irecGesture));
	}
	else 
	{
		if(aGesture.getGestureType() != TWO_HAND)
		{
			gesture noGesture;
			noGesture.GestureName = "Not";
			aGesture.setCurrentGesture(noGesture);
		}
	}
}
...
{
	string filepath = "newdata/test/test";
	Matrix3D data = get_xyz_data("newdata/test", "test");
...
double HMM::viterbi(Matrix1D o, Matrix2D a, Matrix2D b, Matrix1D pi)
{
	...
	// Creation of probability and best path
	double max = -1, temp;
	for(int k = 0; k<K; ++k)
	{
	...
	double prob = 0;
	vector<double> path = X.data;
	// sort and select distinct values to determine if gesture recognised.
	...
	prob = log(prob); // return probability likelihood
...
void HMM::train_hmm(Matrix2D X, Matrix2D pP, Matrix1D bins, int K = 2, int cyc = 100, double tol = 0.0001)
{
	...
	E = cdiv(E, csum(E));	// initiate emission matrix

	Matrix2D B = Matrix2D(((int)TMAX), K);

	Pi = random(K);
	Pi /= Pi.sum();	// initiate initial probabilities

	P = pP;
	P = rdiv(P, rsum(P));	// initiate transmition matrix
	...
	for(int cycle = 0; cycle < cyc; cycle++)
	{
		...
		for(int n = 0; n < N; n++)
		{
			...
			for(size_t i = 0; i < T.data.at(n); i++)
			{
			...
			// forward procedure
			for(size_t i = 1; i < T.data.at(n); i++)
			{
			...
			for(size_t i = 0; i < T.data.at(n)-1; i++)
			{
			...
			for(size_t i = 0; i < T.data.at(n)-1; i++) Scale.data.at(i) += log(scale.data.at(i));
...
void HMM::write_data_to_XML(double threshold, Matrix2D centroids, string Gesture)
{
	...
	for(size_t i = 0; i < centroids.data.size(); i++)
	{
		for(size_t j = 0; j < centroids.data.at(0).data.size(); j++)
		{
			...
			if(j != (centroids.data.at(0).data.size()-1)) tempvar << ",";
		}
	}
	...
	for(size_t i = 0; i < P.data.size(); i++)
	{
		for(size_t j = 0; j < P.data.at(0).data.size(); j++)
		{
			...
			if(j != (P.data.at(0).data.size()-1)) tempvar << ",";
		}
	}
	...
	for(size_t i = 0; i < E.data.size(); i++)
	{
		for(size_t j = 0; j < E.data.at(0).data.size(); j++)
		{
			...
			if(j != (E.data.at(0).data.size()-1)) tempvar << ",";
		}
	}
...
Matrix3D HMM::toMatrix3D(vector<int> X, vector<int> Y, vector<int> Z)
{
	Matrix3D temp;
	...
	{
		Matrix2D temp2d = Matrix2D();
		Matrix1D values = Matrix1D(3);

		values.data.at(0) = X.at(i);
		values.data.at(1) = Y.at(i);
		values.data.at(2) = Z.at(i);

		temp2d.data.push_back(values);
		temp.data.push_back(temp2d);
	}
